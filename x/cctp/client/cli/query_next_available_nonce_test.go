...
func networkWithNextAvailableNonceObjects(t *testing.T) (*network.Network, types.Nonce) {
	...
	nullify.Fill(&NextAvailableNonce)
	state.NextAvailableNonce = NextAvailableNonce
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), *state.NextAvailableNonce
}

func TestShowNextAvailableNonce(t *testing.T) {
	net, obj := networkWithNextAvailableNonceObjects(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowNextAvailableNonce(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetNextAvailableNonceResponse
